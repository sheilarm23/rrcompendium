...
gapsFasta <- read.fasta(file = "../data/raw_data/Sicydium_COI_alignment.fasta", seqtype = "DNA", as.string = TRUE, forceDNAtolower = FALSE)
...
gettingRF <- function(x){
  ...
  longestSeq1 <- orf$start[orf$width == max(orf$width)]
  codonPosition1 <- longestSeq[1]/3
  codonConfirmation1 <- round(codonPosition1)
  if (codonPosition1==codonConfirmation1){
    print("The Reading Frame of the seqs is +1, no further action is required, go to modelTesting")
    } else {
    if (codonPosition1!=codonConfirmation1){
      ...
      if (codonPosition2==codonConfirmation2){
        writeXStringSet(newRF2, file="../data/raw_data/Sicydium_COI_newRF2.fasta")
      print("The Reading Frame of the seqs is +2, new fasta file created, go to modelTesting")
      } else {
      if (codonPosition2!=codonConfirmation2){
        #remove first nucleotide position
        tempLength <- length(fasta[[1]])
        newRF3 <- subseq(fasta, start = 3, end = tempLength)
        orf3 <- predORF(newRF3[1:10], n=1, type="df", mode="orf", strand="sense") 
        # checking size gain
        longestSeq3 <- orf3$start[orf3$width == max(orf3$width)]
        codonPosition3 <- (longestSeq3[1]+2)/3
        codonConfirmation3 <- round(codonPosition3)
        if (codonPosition3==codonConfirmation3){
          writeXStringSet(newRF3, file="../data/raw_data/Sicydium_COI_newRF3.fasta")
        print("The Reading Frame of the seqs is +3, new fasta file created, go to modelTesting")
       }
     }
   }  
    }
...
# the alternative is var1 = read.FASTA = (sequences, type = 'DNA')
