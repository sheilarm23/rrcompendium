...
gettingBestModel <- function(x){
  var1 <- read.dna(file="../data/raw_data/Sicydium_COI_newRF.fasta", format = "fasta")
  var2 <- as.phyDat(var1)
  var3 <- modelTest(var2, tree = NULL, model = c("JC", "K80", "HKY", "TrN", "SYM", "GTR"), multicore = TRUE, mc.cores = 2)
  return(var3)
}
...
var11 <- subset(var2, 1:10)
var21 <- phyDat(var11, type = "DNA", levels = NULL)

var3 <- modelTest(var21, tree = NULL, model = c("JC", "K80", "HKY", "TrN", "SYM", "GTR"), multicore = TRUE, mc.cores = 2)

print(var3)


dna_dist <- dist.ml(var11, model="JC69")
mammals_NJ  <- NJ(dna_dist)
plot(mammals_NJ, main = "Neighbor Joining")

fit <- pml(mammals_NJ, var11)
print(fit)
...
# format = can be sequential, interleaved, clustal
