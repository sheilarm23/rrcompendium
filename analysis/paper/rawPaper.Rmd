...
fastaNames <- strsplit(names(fastafile), '_')                  # creating a vector splitting species names
incompleteNames <- c("sp.")                                    # finding names to remove: indexes which are in the species to be removed
completeSeqs <- which(! fastaNames %like% incompleteNames)     # selection those without specific name in them

# saving a new fasta file with only those seqs with full scientific names
write.fasta(sequences=fastafile[completeSeqs], names=names(fastafile)[completeSeqs], file.out="Sicydium_COI_edits.fasta")
